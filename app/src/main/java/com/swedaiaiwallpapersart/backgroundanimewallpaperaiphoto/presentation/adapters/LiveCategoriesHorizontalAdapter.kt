...
class LiveCategoriesHorizontalAdapter(
    var arrayList: ArrayList<CategoryApiModel>,
    ...
    inner class ViewHolderContainer1(val binding: ListItemLiveWallpaperHorizontalBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(
            modela: ArrayList<CategoryApiModel?>,
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolderContainer1 {
        val binding = ListItemLiveWallpaperHorizontalBinding.inflate(
    ...
    private fun setAllData(
        model: CategoryApiModel,
        position: Int,
        image: ShapeableImageView,
        text: TextView
    ) {

        Glide.with(context!!).load(model.categoryImage).diskCacheStrategy(DiskCacheStrategy.DATA)
            .thumbnail(0.1f)
        ...
        image.setOnClickListener {
            ...
            if (currentTime - lastClickTime >= debounceThreshold) {
                model.categoryName?.let { it1 -> stringCallback.getStringCall(it1) }
    ...
    fun updateMoreData(list: ArrayList<CategoryApiModel?>) {
    ...
    fun getAllItems(): ArrayList<CategoryApiModel> {
        return arrayList
    }
