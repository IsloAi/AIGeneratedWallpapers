...
import com.swedaiaiwallpapersart.backgroundanimewallpaperaiphoto.presentation.utils.PostDataOnServer
...
class FeedbackFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<RelativeLayout>(R.id.backButton)
        ...
        view.findViewById<Button>(R.id.sendButton)?.setOnClickListener {
            ...
            if (MySharePreference.getFeedbackValue(requireContext())) {
                Toast.makeText(
                    requireContext(),
                    getString(R.string.you_already_send_feedback_now_you_can_send_again_after_24_hours),
                    Toast.LENGTH_SHORT
                ).show()
            } else {
                if (getMail.isEmpty()) {
                    ...
                    try {
                        lifecycleScope.launch(Dispatchers.IO) {
                            /*endPointsInterface.postData(
                                FeedbackModel(
                                    getMail, getName, getSubject, getMessage,
